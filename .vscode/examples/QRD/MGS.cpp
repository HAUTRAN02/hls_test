        ...
        ihc::stream_out<QrdFloatingPoint> &rMatrixStream)
{
	...
	hls_memory hls_max_replicates(2) hls_singlepump
...
// Without ivdep, the compiler is concerned that the same element of
// t_matrix may be accessed COLS_COMPONENT iterations apart, creating a
// memory dependence that limits performance. Since cols must be <=
// COLS_COMPONENT, this won't happen, and we can use ivdep to tell the
// compiler that there is no dependency.
#pragma ivdep array(t_matrix)
#pragma unroll 1
