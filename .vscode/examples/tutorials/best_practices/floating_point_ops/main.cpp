...
int main(void) {
  ...
  float test_inputs[TEST_INPUTS];
  float expected[TEST_INPUTS];
  float out[TEST_INPUTS];
  ...
  for (int i = 0; i < TEST_INPUTS; ++i) {
    ...
    ihc_hls_enqueue(out+i, &fir_filter, x);
  }
  ihc_hls_component_run_all(fir_filter);

  // Generate expected outputs from golden models
  golden_fir_filter(test_inputs, expected, TEST_INPUTS);
