...
component float fir_filter (float x) {
  ...
  for(int i = 0; i <= (NUM_TAPS - 1); ++i) {
    // These pragmas only apply to the scope within which they are placed. If
    // there are multiple versions of the same pragma applied to a single scope,
    // the innermost one takes precedence. Also, note that these pragmas must be
    // applied to the top of the block scope as in this example here.
