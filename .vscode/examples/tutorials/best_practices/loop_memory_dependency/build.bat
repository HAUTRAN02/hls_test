:: This batch file will compile the example design:
::   1) default      Compile the example design
::   2) clean        Remove any temporary files generated by the compiler
:: Usage: build.bat <target>
:: Example: build.bat default

@ECHO OFF
:: Only 2 arguments expected
if not "%3"=="" goto usage

set NAME=transpose_and_fold

set "TARGET=default"
if not "%1"=="" set "TARGET=%1"

set "DEVICE=Arria10"
if not "%2"=="" set "DEVICE=%2"

if "%TARGET%" == "default" (
  echo i++ -march=%DEVICE% %NAME%.cpp -o %NAME%_no_ivdep
  i++ -march=%DEVICE% %NAME%.cpp -o %NAME%_no_ivdep
  if not ERRORLEVEL 0 (
    echo Error: Compile failed
    exit /b 1
  )

  echo i++ -march=%DEVICE% -DUSE_IVDEP %NAME%.cpp -o %NAME%_ivdep
  i++ -march=%DEVICE% -DUSE_IVDEP %NAME%.cpp -o %NAME%_ivdep
  if not ERRORLEVEL 0 (
    echo Error: Compile failed
    exit /b 1
  )

  echo %NAME%_no_ivdep.exe
  %NAME%_no_ivdep.exe

  echo %NAME%_ivdep.exe
  %NAME%_ivdep.exe

  echo Compilation complete.  Open the README and the report files to examine the impact of using the ivdep pragma.
  goto:eof

) else if "%TARGET%" == "clean" (
  del /s /f *.tmp
  del /s /f *.exe
  del /s /f *.obj
  rmdir /s /q %NAME%_no_ivdep.prj 2>nul
  rmdir /s /q %NAME%_ivdep.prj 2>nul
  goto:eof
) else (
  goto usage
)

:: Dump the usage if we get unexpected input
:usage
echo Usage: build.bat [target]
echo Targets: default, clean
echo Example: build.bat default
exit /b 2
