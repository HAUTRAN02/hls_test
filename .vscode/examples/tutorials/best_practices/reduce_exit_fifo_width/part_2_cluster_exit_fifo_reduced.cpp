...
void component design_v2(
    ...
    // Optimzation: Merge the loops
    for (int i=0; i<N+KEY_RANGE; i++) {
        // For the first N iterations: read in the key-value pairs
        ...
        if (i<N) {
            ...
            // compile time. This allows the compiler to generate
            // faster hardware at the expense of some area. See the
            // tutorial on parallelizing array operations for more
            // details
            #pragma unroll KEY_RANGE
            ...
            // For the each one of the last KEY_RANGE iterations,
            // emit the key-value pair at index 0.
            B.write(std::make_pair(i-N, accumulators[0]));
            
            // Shift each element left by one position, starting from
            // the first element
            #pragma unroll
