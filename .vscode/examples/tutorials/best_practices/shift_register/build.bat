...
if "%TARGET%" == "default" (
  ...
  echo i++ -march=x86-64 moving_avg.cpp -o tutorial-x86.exe
  i++ -march=x86-64 moving_avg.cpp -o tutorial-x86.exe
  if not ERRORLEVEL 0 (
    echo Error: Compile failed
    exit /b 1
  )

  echo i++ -march=%DEVICE% moving_avg.cpp -o tutorial-fpga.exe
  i++ -march=%DEVICE% moving_avg.cpp -o tutorial-fpga.exe
  if not ERRORLEVEL 0 (
    echo Error: Compile failed
    exit /b 1
  )
