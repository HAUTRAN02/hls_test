...
component void accum(int numMatrices, ihc::stream_in<float> &matrixInput, ihc::stream_out<float> &matrixOutput) {
  ...
  hls_register float resultBuffer1[NUM_ROWS][NUM_COLS] = {0.0f}; 
  hls_register float resultBuffer2[NUM_ROWS][NUM_COLS] = {0.0f};
  ...
  for (int i = 0; i < NUM_ROWS; i++) {
    resultBuffer1[i][i] = 1.0f;
    resultBuffer2[i][i] = 1.0f;
  }
  ...
  for (int row = 0; row < NUM_ROWS; row++) {
    for (int col = 0; col < NUM_COLS; col++) {
      matrixOutput.write(resultBufferPtr2[row*NUM_COLS + col]);
    }
  }
}
...
int main() {
  ...
  printf("Calling matrix multiplication with pointer swapping and HLS registers \n");
  ...
  // neither will be printed due to the compiler error
  if (passed) {
    printf("PASSED\n");
  } else {
    printf("FAILED\n");
