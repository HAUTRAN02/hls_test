...
component int sum(hls_avalon_slave_memory_argument(BUFFER_SIZE) hls_numbanks(1)
                      hls_bankwidth(16) int *mmBuffer) {
  int result = 0;
...
int main() {
  ...
  for (int k = 0; k < NUM_TESTS; k++) {
    ...
    for (int i = 0; i < ARR_SIZE; i++) {
      mmBuffer[i] = mmBufferRef[i] = rand() % MAXVAL;
    }
