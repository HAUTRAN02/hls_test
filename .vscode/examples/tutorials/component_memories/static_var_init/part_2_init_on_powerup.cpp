...
component int counter(int x) {
  hls_init_on_powerup hls_memory_impl("BLOCK_RAM") static int counter_arr[ARR_SIZE] = 
  {
    0, 1,  2,  3,  4,  5,  6,  7, 
...
int main(){
  ...
  if (ihc_hls_sim_reset()) {
    // If this is testing the FPGA components in simulation, check that statics are not reinitialized
    for (int i = 0; i < ARR_SIZE; i++)
      pass &= (counter(i) == counter_ref(i));
  }
