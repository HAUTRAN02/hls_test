...
typedef ihc::stream_in<four_pixels, ihc::bitsPerSymbol<8>,
                       ihc::firstSymbolInHighOrderBits<false>>
    stream_in_pixels;
typedef ihc::stream_out<four_pixels, ihc::bitsPerSymbol<8>,
                        ihc::firstSymbolInHighOrderBits<false>>
    stream_out_pixels;
...
component void redden(stream_in_pixels &in_pixels,
                      stream_out_pixels &out_pixels, unsigned num_pixels) {
  unsigned num_beats = (num_pixels + PIXELS_PER_BEAT - 1) /
                       PIXELS_PER_BEAT; // equivalent to ceil((double)
                                        // num_pixels/PIXELS_PER_BEAT)
...
int main(void) {
  ...
  for (unsigned beat = 0; beat < num_beats; ++beat) {
    ...
    for (unsigned pixel_offset = 0;
         pixel_offset < ((beat == num_beats - 1) ? num_pixels % PIXELS_PER_BEAT
                                                 : PIXELS_PER_BEAT);
         ++pixel_offset) {
  ...
  // check that R symbols are set to 0xff if the green or blue symbols exceed
  // the threshold
  bool passed = true;
