...
typedef ihc::stream_in<four_pixels, ihc::usesPackets<true>,
                       ihc::bitsPerSymbol<8>, ihc::usesEmpty<true>,
                       ihc::firstSymbolInHighOrderBits<true>>
    stream_in_pixels;
typedef ihc::stream_out<four_pixels, ihc::usesPackets<true>,
                        ihc::bitsPerSymbol<8>, ihc::usesEmpty<true>,
                        ihc::firstSymbolInHighOrderBits<true>>
    stream_out_pixels;
...
void redden_pixel(four_pixels &pixels, unsigned shift_amount) {
  symbol blue =
      (pixels & ((four_pixels)0x00ff00 << shift_amount)) >> shift_amount;
  symbol green =
      (pixels & ((four_pixels)0xff0000 << shift_amount)) >> shift_amount;
  if (blue > 200 || green > 200) {
    pixels |= (((four_pixels)0x0000ff) << shift_amount); // R symbol = 0xff
  }
}

component void redden(stream_in_pixels &in_pixels,
  ...
  while (!end_of_packet) {
    ...
    // set valid R symbols to 0xff if the blue or green symbols exceed the
    // threshold pixel 0
...
int main(void) {
  ...
  for (unsigned beat = 0; beat < num_beats; ++beat) {
    ...
    int empty = 6; // the number of invalid symbols must be a multiplication of
                   // the number of symbols per pixel in all circumstances, e.g.
                   // 2 empty pixels * 3 symbols per pixel

    four_pixels pixels = 0;
    for (unsigned pixel_offset = 0;
         pixel_offset <
         (end_of_packet ? empty / SYMBOLS_PER_PIXEL : PIXELS_PER_BEAT);
         ++pixel_offset) {
  ...
  // check that valid R symbols are set to 0xff if the green or blue symbols
  ...
  for (unsigned beat = 0; beat < num_beats; ++beat)
  {
    bool start_of_packet_expected = (beat == 0);
    bool end_of_packet_expected = (beat == num_beats - 1);
    int empty_expected = 6;

    four_pixels pixels =
        out_pixels.read(start_of_packet, end_of_packet, empty_out);

    if ((start_of_packet != start_of_packet_expected) ||
        (end_of_packet != end_of_packet_expected) ||
        (empty_out != empty_expected))
    {
      printf("beat %d: SOP = %s (expected %s) EOP = %s (expected %s) empty = %d (expected %d)\n", 
              beat,
              start_of_packet ? "TRUE" : "FALSE", 
              start_of_packet_expected ? "TRUE" : "FALSE",
              end_of_packet ? "TRUE" : "FALSE", 
              end_of_packet_expected ? "TRUE" : "FALSE",
              empty_out, empty_expected);
      passed = false;
    }
    ...
    for (unsigned pixel_offset = 0;
         pixel_offset <
         (end_of_packet ? empty_out / SYMBOLS_PER_PIXEL : PIXELS_PER_BEAT);
         ++pixel_offset) {
      unsigned shift_amount = pixel_offset * BITS_PER_PIXEL;
      symbol blue =
          (pixels & ((four_pixels)0x00ff00 << shift_amount)) >> shift_amount;
      ...
      if (blue > 200 || green > 200) {
        passed &= ((pixels |= (((four_pixels)0x0000ff) << shift_amount)) ==
                   pixels); // R symbol = 0xff
      }
    }
  }
