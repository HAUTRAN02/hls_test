...
typedef mm_host<unsigned int, dwidth<32>, awidth<64>, aspace<1>, latency<0>, maxburst<1> > mm_src_t;
typedef mm_host<unsigned int, dwidth<32>, awidth<64>, aspace<3>, latency<0>, maxburst<1> > mm_dst_t;
...
int main () {
  ...
  /*
    1)
    This code creates a new mm_host interface for each iteration of the loop.
  */
  x = 0x10;
  for (int i=0; i<NUM_SAMPLES; ++i) {
  ...
  dump_buffers("New mm_host for each index", src_mem, dst_mem, NUM_SAMPLES);
  ...
  /*
    ...
    be used to index into a mm_host object.  This can be useful when iterating
