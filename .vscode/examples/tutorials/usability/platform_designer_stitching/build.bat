...
if "%TARGET%" == "default" (
  echo i++ -march=%DEVICE% --simulator none bandpassfilter.cpp -o tutorial-fpga.exe
  i++ -march=%DEVICE% --simulator none bandpassfilter.cpp -o tutorial-fpga.exe
  if not ERRORLEVEL 0 (
  ...
  echo qsys-generate top.qsys --simulation=VERILOG ^> qsys-generate.log 2^>^&1
	qsys-generate top.qsys --simulation=VERILOG > qsys-generate.log 2>&1
	echo vsim -batch -do test.tcl ^> vsim.log 2^>^&1
