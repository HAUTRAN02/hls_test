    ...
    for (row = 0; row < 28; row++)
    {
      for (col = 0; col < 28; col++)
      {
        for (i = 0; i < 1; i++)
        {
          for (j = 0; j < 1; j++)
          {
             if (i==0 && j==0) o_conv1[28*28* channel + 28 * row + col]  = input0[(row + i) * 28 + (col + j)] * w_conv1[channel*1*1 +1*i+j] + b_conv1[channel];
           else              o_conv1[28*28* channel + 28 * row + col] += input0[(row + i) * 28 + (col + j)] * w_conv1[channel*1*1 +1*i+j];
          }
        }
      }
    }
...
component void relu1(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(6*28*28 * sizeof(float)) float *input1,
   ...
   for(k=0;k<6;k++){
       for(i=0;i<28;i++){
           for(j=0;j<28;j++){
               output1[k][i][j] = (input1[k][i][j] < 0.0f)? 0.0f: input1[k][i][j];
           }
       }
   }
}
...
component void avgpooling1(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(6*28*28 * sizeof(float)) float *input1,
   ...
   for(n_channel=0;n_channel<6;n_channel++){
       for(i=0;i<28;i+=2){
           for(j=0;j<28;j+=2){
               output1[n_channel][i/2][j/2] = (input1[n_channel][i][j] + input1[n_channel][i+1][j] + input1[n_channel][i][j+1] + input1[n_channel][i+1][j+1])/ (4.0f);
           }s
       }
   }
}
   ...
   for(row=0;row<10;row++){
     for(col=0;col<10;col++){
       for(k=0;k<6;k++){
         for(i=0;i<5;i++){
           for(j=0;j<5;j++){
               if (k==0 && i==0 && j==0) output2[channel][row][col]  = input2[k][row+i][col+j] * kernel[channel][k][i][j] + bias[channel];
               else                      output2[channel][row][col] += input2[k][row+i][col+j] * kernel[channel][k][i][j];
           }
         }
       }
     }
...
component void relu2(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(16*10*10  * sizeof(float)) float *input3,
   ...
   for(k=0;k<16;k++){
       for(i=0;i<10;i++){
           for(j=0;j<10;j++){
               output3[k][i][j] = (input3[k][i][j] < 0.0f)? 0.0f: input3[k][i][j];
           }
       }
   }
}
...
component void avgpooling2(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(16*10*10  * sizeof(float)) float *input4,
    ...
    float in[16][10][10], float out[16][5][5]){
   int n_channel, i, j;
   for(n_channel=0;n_channel<16;n_channel++){
       for(i=0;i<10;i+=2){
           for(j=0;j<10;j+=2){
               output4[n_channel][i/2][j/2] = (input4[n_channel][i][j] + input4[n_channel][i+1][j] + input4[n_channel][i][j+1] + input4[n_channel][i+1][j+1]) / (4.0f);
           }
       }
   }
}
...
component void fc1(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(400  * sizeof(float)) float *input6,
    ...
    float in[400], float weights[120][400], float bias[120], float out[120]){
   int i,j;
   for(i=0;i<120;i++){
       for(j=0;j<400;j++){
           if(j==0)    output6[i]  = (weights[i][j] * in[j])  + bias[i];
           else        output6[i] +=  weights[i][j] * in[j];
       }
   }
}
...
component void fc2(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(120 * sizeof(float)) float *input8,
   ...
   for(i=0;i<84;i++){
       for(j=0;j<120;j++){
           if(j==0)    output8[i]  =  (weights[i][j] * input8[j]) + bias[i];
           else        output8[i] +=   weights[i][j] * input8[j];
       }
   }
}
...
component void fc3(hls_numbanks(1) hls_singlepump hls_avalon_slave_memory_argument(84 * sizeof(float)) float *input10,
   ...
   for(i=0;i<10;i++){
       for(j=0;j<84;j++){
           if(j==0)    output10[i]  = (weights[i][j] * input10[j]) + bias[i];
           else        output10[i] +=  weights[i][j] * input10[j];
       }
   }
}
